(()=> {
    ...
    window.addEventListener('load', () => {
        const respuestaCorrectaInt = document.getElementById('respuestaCorrectaT');
        const respuestaErroneaInt = document.getElementById('respuestaErroneaT');
        respuestaCorrectaInt.style.display = 'none';
        respuestaErroneaInt.style.display = 'none';
        // Fetch all the forms we want to apply custom Bootstrap validation styles to
        var formsInte = document.getElementsByClassName('Tocada');
        ...
        var validation = Array.prototype.filter.call(formsInte, (form) => {
            form.addEventListener('submit',  (event) => {
                ...
                if (form.checkValidity()) {
                    let formRegistrar = document.getElementById('Tocada');
                    let dataForm = new FormData(formRegistrar);
                    fetch('modulos/php/modificarTocada.php',{
                        method : 'POST',
                        body : dataForm
                    })
